	...
	#Replace Az with AzureRm is you are using AzureRM Model of Azure PowerShell 
	
    Connect-AzAccount -DeviceCode
	Get-azSubscription
	$subscriptionID = "04d3fbff-1060-4e60-b833-ae7b6e290880"
	Select-azSubscription -Subscription $subscriptionID

    $rbacName = "Spektra Custom RBAC"
	<# Name you provide in $rbacName will be written in Rbac name field as shown below
	{
	  "Name": "AVD-AZ-140-Custom-Role",
	  "Id": null,#>
	...
	{ 
	    ...
	    Write-Host "======================================================="
		$resources = Get-azResource -oDataQuery "`$filter=resourcegroup eq '$RGName'"
	...
	<#-----------------------------------------------------------------------------------------------------------------------------------------#>
	  ...
	  if($sqlServers)
	  {
	    ...
	    { 
			...
			$sqlDatabases = Get-azSqlDatabase -ResourceGroupName $RGName -ServerName $sqlServerName
			if($sqlDatabases)
		    {
    ...
    # VM Scale Set
    $vmScaleSets = Get-azVmss -ResourceGroupName $RGName
	  if($vmScaleSets)
		  {
    ...
    # Load Balancer
    $lbs = Get-azLoadBalancer -ResourceGroupName $RGName
	  if($lbs)
		  {
            $lbSKUNameArray = @()
            $lbSKUNameArray1 = @()
    ...
    # Application Gateway
    $appGateways = Get-azApplicationGateway -ResourceGroupName $RGName
	  if($appGateways)
		  {
    ...
    $expressRoutes = Get-azExpressRouteCircuit -ResourceGroupName $RGName
	  if($expressRoutes)
		  {
    ...
    # Cognitive Services
    $cognitiveServiceAccounts = Get-azCognitiveServicesAccount -ResourceGroupName $RGName
    ...
    # Container Registry
    $containerRegistries = Get-azContainerRegistry -ResourceGroupName $RGName
    ...
    # Event Hub
    $eventHubs = Get-azEventHubNamespace -ResourceGroupName $RGName 
    if($eventHubs)
    {
        $eventHubSKUArray = @()
        $eventHubSKUArray1 = @()
